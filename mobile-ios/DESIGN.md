...
| The gateway falls back from ICMP to TCP | Section 5.6 assumed the router would answer echo requests. A campus router did not, with the address confirmed correct and permission granted. A TCP **refusal** is still a round trip, since an RST comes from the host's own stack, so the port does not need to be open: the router only has to answer something. |
